  ...
  fetch(corsProxy + url)
    ...
    .then((data) => {
      ...
      try {
        ...
        errorMessage.textContent =
          "Ошибка парсинга XML: Некорректный формат RSS.";
        resultDiv.classList.add("invalid");
        feedsContainer.innerHTML = "";
        return;
      }
      ...
      if (!rssElement) {
        ...
        if (resultDiv && resultDiv.classList) {
          resultDiv.classList.add("invalid");
        } else {
          console.error("Ошибка: resultDiv не найден или не имеет classList!");
        }
    ...
    if (resultDiv && resultDiv.classList) {
      resultDiv.classList.add("invalid"); // Добавила "invalid"
  } else {
      console.error("Ошибка: resultDiv не найден или не имеет classList!");
  }
  feedsContainer.innerHTML = '';
